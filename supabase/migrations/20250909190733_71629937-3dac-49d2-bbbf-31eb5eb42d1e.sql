...
create policy "Authorities can view and update all incidents"
  on public.incidents
  for select using (has_role(auth.uid(), 'authority') or has_role(auth.uid(), 'admin'))
  for update using (has_role(auth.uid(), 'authority') or has_role(auth.uid(), 'admin'));
...
create policy "Authorities can create alerts for any user and view all"
  on public.alerts
